...
router.get('/', async (req, res) => {
  try {
    let allPost = await Post.findAll();//add options to limit quantity
    const posts =  allPost.map((item) => {
    ...
    res.render('homepage',{
      posts // posts:posts
      //this will be an array of post from the database
      //rename dates variable to something appropriate like posts
    });
...
router.post('/submit', async (req, res) => {
  try {
    ...
    res.redirect('/')// Redirect to a success page after successful submission
  } catch (error) {
    console.error(error);
...
router.get('/schedule', async (req, res) => {
  ...
  try {
    ...
    res.render('dashboard', { dates },);
  } catch (err) {
    console.log(err, 'Error directing to your dashboard');
    res.status(500).json(err);
  }
});
